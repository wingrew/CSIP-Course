...
class Config:
    ...
    def find_usable_node(self, need):
        ...
        node_ret = None
        if self.lock_nodes.acquire():
            for node, all_t in self.nodes.items():
...
def recv_job(jdatparm):
    ...
    try:
        infos = jdatparm[0]
        cmd = jdatparm[1]
        if not infos.get('threads'):
            infos['threads']=1
        # 使用正则表达式提取 JSON 部分
        json_part = re.search(r'\{.*\}', cmd)
        if json_part:
            print(json_part)
            json_part = json_part.group()
            data = json.loads(json_part)
            # 提取 "threads" 的值
            real_threads = int(data["threads"])
        infos['threads']=real_threads
    ...
    if "snakemake" in cmd:
        cmd = "{} cd {}; source ~/.bashrc; {}".format(config.cmd_prefix, pwd, cmd)
    else:
